...
func NewGost(chainNodes, serveNodes StringList) *GostRoutes {
	var gostRoute GostRoutes
	var options = route{
	...
	if len(options.ServeNodes) > 0 {
		gostRoute.routes = append(gostRoute.routes, options)
	}
	gost.Debug = false

	if len(gostRoute.routes) == 0 {
		os.Exit(0)
	}
